...
test('deletable prop', () => {
  ...
  expect(wrapper.find('.ca2-uploader__preview-delete').element).toBeTruthy();

  wrapper.setProps({ deletable: false });
  expect(wrapper.find('.ca2-uploader__preview-delete').element).toBeFalsy();
});
...
test('delete preview image', () => {
  ...
  wrapper.find('.ca2-uploader__preview-delete').trigger('click');
...
test('click to preview image', async () => {
  ...
  wrapper.find('.ca2-image').trigger('click');
  const imagePreviewNode = document.querySelector('.ca2-image-preview');
  ...
  const imagePreviewNode2 = document.querySelector('.ca2-image-preview');
  const images = imagePreviewNode2.querySelectorAll(
    '.ca2-image-preview__image'
  );
...
test('close-preview event', async () => {
  ...
  const preview = document.querySelector('.ca2-image-preview');
  const swipe = preview.querySelector('.ca2-swipe__track');
...
test('show-upload prop', () => {
  const wrapper = mount(Uploader);
  expect(wrapper.contains('.ca2-uploader__upload')).toBeTruthy();
  wrapper.setProps({ showUpload: false });
  expect(wrapper.contains('.ca2-uploader__upload')).toBeFalsy();
});
