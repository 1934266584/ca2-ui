...
<ca2-cell-group>
  <ca2-field v-model="value" placeholder="请输入用户名" />
</ca2-cell-group>
```
...
```html
<!-- 输入任意文本 -->
<ca2-field v-model="text" label="文本" />
<!-- 输入手机号，调起手机号键盘 -->
<ca2-field v-model="tel" type="tel" label="手机号" />
<!-- 允许输入正整数，调起纯数字键盘 -->
<ca2-field v-model="digit" type="digit" label="整数" />
<!-- 允许输入数字，调起带符号的纯数字键盘 -->
<ca2-field v-model="number" type="number" label="数字" />
<!-- 输入密码 -->
<ca2-field v-model="password" type="password" label="密码" />
```
...
```html
<ca2-cell-group>
  <ca2-field label="文本" value="输入框只读" readonly />
  <ca2-field label="文本" value="输入框已禁用" disabled />
</ca2-cell-group>
```
...
>
  <template #button>
    <ca2-button size="small" type="primary">发送验证码</ca2-button>
  </template>
</ca2-field>
```
...
| input-align | 输入框对齐方式，可选值为 `center` `left` | *string* | `right` |
