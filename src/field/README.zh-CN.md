...
<zv-cell-group>
  <zv-field v-model="value" placeholder="请输入用户名" />
</zv-cell-group>
```
...
```html
<!-- 输入任意文本 -->
<zv-field v-model="text" label="文本" />
<!-- 输入手机号，调起手机号键盘 -->
<zv-field v-model="tel" type="tel" label="手机号" />
<!-- 允许输入正整数，调起纯数字键盘 -->
<zv-field v-model="digit" type="digit" label="整数" />
<!-- 允许输入数字，调起带符号的纯数字键盘 -->
<zv-field v-model="number" type="number" label="数字" />
<!-- 输入密码 -->
<zv-field v-model="password" type="password" label="密码" />
```
...
```html
<zv-cell-group>
  <zv-field label="文本" value="输入框只读" readonly />
  <zv-field label="文本" value="输入框已禁用" disabled />
</zv-cell-group>
```
...
>
  <template #button>
    <zv-button size="small" type="primary">发送验证码</zv-button>
  </template>
</zv-field>
```
