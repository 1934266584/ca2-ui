...
test('simulation finger swipe again before transitionend', () => {
  ...
  triggerDrag(wrapper.find('.ca2-picker-column'), 0, -5);
  triggerDrag(wrapper.find('.ca2-picker-column'), -5, -100);
  wrapper.find('.ca2-picker-column ul').trigger('transitionend');
  expect(wrapper.emitted('change')[0][1]).toEqual('1995');
});
...
test('click column item', () => {
  ...
  wrapper.findAll('.ca2-picker-column__item').at(3).trigger('click');
